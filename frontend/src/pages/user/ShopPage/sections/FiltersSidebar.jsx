...
export default function FiltersSidebar(props) {
    const {
        ...
        allGenres, selectedGenres, handleGenreChange, genreCounts,
        sortBy, setSortBy,
        mobileFiltersOpen
    } = props
    ...
    return (
        <div
            className={`w-full md:w-58 shrink-0 rounded-lg p-8 h-fit bg-[#F4F4F4] border-gray-400 drop-shadow-lg border-2 md:block md:sticky md:top-16 ${mobileFiltersOpen ? 'block' : 'hidden'}`}>
            <ConditionFilter
            ...
            <div className="h-px bg-gray-200 my-4"></div>
            <GenreFilter allGenres={allGenres} selectedGenres={selectedGenres} handleGenreChange={handleGenreChange} genreCounts={genreCounts}/>
            <div className="h-px bg-gray-200 my-4"></div>
