...
export default function OrderConfirmationPage() {
    ...
    const { orderDetails, paymentMethod, shippingAddress: flatShippingAddress } = location.state || {};
    ...
    if (!orderDetails || !orderDetails.id) {
        return (
            <div className="flex justify-center items-center min-h-screen bg-slate-100">
                <p className="text-lg text-gray-700">Loading order details or invalid access...</p>
            </div>
        );
    }

    const itemsSubtotal = orderDetails.items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    ...
    const paymentColorClasses = statusTagColors[paymentStatus] || statusTagColors.UNKNOWN;

    const currentOrderStatus = orderDetails.orderStatus || 'UNKNOWN';
    const orderStatusColorClasses = statusTagColors[currentOrderStatus] || statusTagColors.UNKNOWN;
            ...
            <div className="max-w-screen-lg mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white shadow-2xl rounded-lg p-6 sm:p-8 md:p-12 mt-8">
                    <div className="text-center border-b border-gray-200 pb-8 mb-10">
                        ...
                        <h1 className="text-2xl sm:text-3xl font-bold text-[#1C387F]">Thank You For Your Order!</h1>
                        <p className="text-base text-gray-600 mt-3">Your order has been placed successfully.</p>
                        <p className="text-lg text-gray-800 font-semibold mt-4">
                            Order ID: <span className="text-[#1C387F]">{orderDetails.id}</span>
                        </p>
                        <p className="text-sm text-gray-500 mt-2">Order Date: {formatDate(orderDetails.addedAt)}</p>
                    </div>
                        ...
                        <div className="lg:col-span-4">
                            <h2 className="text-xl font-semibold text-gray-800 mb-6 flex items-center justify-between">
                                <span className="flex items-center">
                                    <Package size={24} className="mr-3 text-[#1C387F]" /> Order Summary
                                </span>
                                <span
                            ...
                            <div className="space-y-4 mb-8 max-h-[30rem] overflow-y-auto pr-3 custom-scrollbar">
                                {orderDetails.items.map((item, index) => (
                                    <div key={item.bookId || index} className="flex items-start justify-between p-4 bg-slate-50 rounded-md shadow-sm">
                                        <div className="flex items-start">
                                            ...
                                            />
                                            <div>
                                                <p className="font-semibold text-gray-700 text-sm leading-tight">{item.title}</p>
                            ...
                            <div className="space-y-3 text-sm border-t border-gray-200 pt-6">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-gray-800">{formatCurrency(itemsSubtotal)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-600">Shipping & Taxes:</span>
                                    <span className="font-medium text-gray-800">{formatCurrency(shippingAndTaxes)}</span>
                                </div>
                                <div className="flex justify-between text-lg font-bold text-[#1C387F] mt-2 pt-2 border-t border-dashed">
                        ...
                        <div className="lg:col-span-3 space-y-8">
                            <div>
                                <h2 className="text-xl font-semibold text-gray-800 mb-5 flex items-center">
                                    <CreditCard size={24} className="mr-3 text-[#1C387F]" /> Shipping Address
                                </h2>
                                <div className="text-sm text-gray-700 bg-slate-50 p-5 rounded-md shadow-sm leading-relaxed">
                                    <p className="font-semibold text-[#1C387F]">
                                        {flatShippingAddress.firstName} {flatShippingAddress.lastName}
                                    </p>
                                    <p>{flatShippingAddress.streetAddress}</p>
                                    <p>
                                        {flatShippingAddress.city}, {flatShippingAddress.state} {flatShippingAddress.postalCode}
                                    </p>
                                    <p>{flatShippingAddress.country}</p>
                                    <div className="mt-2 pt-2 border-t border-gray-200">
                                        <p>
                                            <span className="text-gray-500">Email:</span> {flatShippingAddress.email}
                                        </p>
                                        <p>
                                            <span className="text-gray-500">Phone:</span> {flatShippingAddress.phoneNumber}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <h2 className="text-xl font-semibold text-gray-800 mb-5 flex items-center">
                                    <CreditCard size={24} className="mr-3 text-[#1C387F]" /> Payment Information
                                </h2>
                                <div className="text-sm text-gray-700 bg-slate-50 p-5 rounded-md shadow-sm leading-relaxed">
                                    <p>
                                        Payment Method:{" "}
                                    ...
                                    {latestTransaction && (
                                        <p className="mt-1">
                                            Payment Status:{" "}
                                            <span className={`font-semibold ${paymentColorClasses.text} rounded-full px-3 py-1`}
                                                  style={{ backgroundColor: paymentColorClasses.bgColor }}>
                                                {paymentStatus}
                                            </span>
                                        </p>
                                    )}
                    ...
                    <div className="mt-12 text-center border-t border-gray-200 pt-10">
                        <p className="text-sm text-gray-600 mb-8">
                            ...
                            <span className="font-medium text-[#1C387F]">{flatShippingAddress.email}</span>.
                        </p>
                        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 md:gap-6">
                            <Link
                                to="/shop"
                                replace={true}
                                className="w-full sm:w-auto bg-[#1C387F] text-white px-10 py-3 rounded-lg font-semibold hover:bg-blue-800 transition-all duration-300 ease-in-out focus:outline-none focus:ring-4 focus:ring-blue-300/50 flex items-center justify-center gap-2 text-base shadow-md hover:shadow-lg"
                            >
